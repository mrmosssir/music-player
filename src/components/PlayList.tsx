...
const Playlist = function () {
    ...
    const togglePlayList = function (id: string, idx: number) {
        if (idx === activeIdx) {
            setActiveIdx(null);
            return;
        }
    ...
    const getMinute = function (time: number) {
        const minute = Math.floor(time / 60000);
        return minute < 10 ? `0${minute}` : minute;
    }

    const getSecond = function (time: number) {
        ...
        <div id="playlist">
            {playList.map((item, index) => {
                return (
                    <div key={item.id}>
                        <button
                            className="flex items-center px-8 mb-2"
                        ...
                        {tracks.map((track) => {
                            if (activeIdx === index) {
                                return (
                                    <div key={track.id} className="relative h-16 flex justify-between items-center py-2 px-4 border-b border-b-primary-500 text-sm bg-primary-300">
